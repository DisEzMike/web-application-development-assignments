...
export const getDroneConfig = async (droneId) => {
	if (!process.env.DRONE_CONFIG_URL) throw new Error('DRONE_CONFIG_URL is required');
...
export const getDroneLogsById = async (droneId, page = 1) => {
	...
	if (!process.env.DRONE_LOG_TOKEN) throw new Error('DRONE_LOG_TOKEN is required');
